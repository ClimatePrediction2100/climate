...
# Step 1: Generate the nginx.conf file with the provided domains
...
EOF
echo "nginx.conf has been generated with the provided domains."
...
docker exec $(docker ps --filter name=ssl-stack_dummy-certbot -q) certbot certonly --webroot -w /var/www/certbot "${domains_args[@]}" --email $EMAIL --agree-tos --no-eff-email --force-renewal

# Step 6: Remove the ssl-stack stack
docker stack rm ssl-stack
echo "SSL certificate has been issued and the stack has been removed."